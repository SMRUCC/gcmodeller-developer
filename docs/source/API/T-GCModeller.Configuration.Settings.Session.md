...
# Session
_namespace: [GCModeller.Configuration.Settings](N-GCModeller.Configuration.Settings.html)_
...
#### Finallize
```csharp
GCModeller.Configuration.Settings.Session.Finallize
```
Close the application session and save the settings file.

#### FolkShoalThread
```csharp
GCModeller.Configuration.Settings.Session.FolkShoalThread(System.String,System.String)
```
...
#### Initialize
```csharp
GCModeller.Configuration.Settings.Session.Initialize(System.Type)
```
Initialize the application session and get the program profile data.(初始化应用程序会话，并获取应用程序的配置数据)

#### TryGetShoalShellBin
```csharp
GCModeller.Configuration.Settings.Session.TryGetShoalShellBin
```
