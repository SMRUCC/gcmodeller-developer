...
# Session
_namespace: [LANS.SystemsBiology.GCModeller.Settings](N-LANS.SystemsBiology.GCModeller.Settings.html)_
...
#### Finallize
```csharp
LANS.SystemsBiology.GCModeller.Settings.Session.Finallize
```
Close the application session and save the settings file.

#### FolkShoalThread
```csharp
LANS.SystemsBiology.GCModeller.Settings.Session.FolkShoalThread(System.String,System.String)
```
...
#### Initialize
```csharp
LANS.SystemsBiology.GCModeller.Settings.Session.Initialize(System.Type)
```
Initialize the application session and get the program profile data.(初始化应用程序会话，并获取应用程序的配置数据)

#### TryGetShoalShellBin
```csharp
LANS.SystemsBiology.GCModeller.Settings.Session.TryGetShoalShellBin
```
