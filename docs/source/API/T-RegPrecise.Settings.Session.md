...
# Session
_namespace: [RegPrecise.Settings](N-RegPrecise.Settings.html)_
...
#### Finallize
```csharp
RegPrecise.Settings.Session.Finallize
```
Close the application session and save the settings file.

#### FolkShoalThread
```csharp
RegPrecise.Settings.Session.FolkShoalThread(System.String,System.String)
```
...
#### Initialize
```csharp
RegPrecise.Settings.Session.Initialize(System.Type)
```
Initialize the application session and get the program profile data.(初始化应用程序会话，并获取应用程序的配置数据)

#### TryGetShoalShellBin
```csharp
RegPrecise.Settings.Session.TryGetShoalShellBin
```
