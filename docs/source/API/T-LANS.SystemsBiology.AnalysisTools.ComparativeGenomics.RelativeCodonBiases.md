...
#### #ctor
```csharp
LANS.SystemsBiology.AnalysisTools.ComparativeGenomics.RelativeCodonBiases.#ctor(LANS.SystemsBiology.SequenceModel.FASTA.FastaToken)
```
...
```csharp
LANS.SystemsBiology.AnalysisTools.ComparativeGenomics.RelativeCodonBiases.__staticsOfMaxFrequencyCodon(System.Char)
```
统计某一中氨基酸的编码偏好性
...
#### Normalization
```csharp
LANS.SystemsBiology.AnalysisTools.ComparativeGenomics.RelativeCodonBiases.Normalization(Microsoft.VisualBasic.ComponentModel.TripleKeyValuesPair{System.Double})
```
对Profile进行归一化处理

#### W
```csharp
LANS.SystemsBiology.AnalysisTools.ComparativeGenomics.RelativeCodonBiases.W(LANS.SystemsBiology.SequenceModel.NucleotideModels.Translation.Codon)
```
计算 W(Codon)
