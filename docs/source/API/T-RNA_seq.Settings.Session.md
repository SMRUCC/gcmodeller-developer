...
# Session
_namespace: [RNA_seq.Settings](N-RNA_seq.Settings.html)_
...
#### Finallize
```csharp
RNA_seq.Settings.Session.Finallize
```
Close the application session and save the settings file.

#### FolkShoalThread
```csharp
RNA_seq.Settings.Session.FolkShoalThread(System.String,System.String)
```
...
#### Initialize
```csharp
RNA_seq.Settings.Session.Initialize(System.Type)
```
Initialize the application session and get the program profile data.(初始化应用程序会话，并获取应用程序的配置数据)

#### TryGetShoalShellBin
```csharp
RNA_seq.Settings.Session.TryGetShoalShellBin
```
