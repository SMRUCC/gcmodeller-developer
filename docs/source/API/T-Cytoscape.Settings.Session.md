...
# Session
_namespace: [Cytoscape.Settings](N-Cytoscape.Settings.html)_
...
#### Finallize
```csharp
Cytoscape.Settings.Session.Finallize
```
Close the application session and save the settings file.

#### FolkShoalThread
```csharp
Cytoscape.Settings.Session.FolkShoalThread(System.String,System.String)
```
...
#### Initialize
```csharp
Cytoscape.Settings.Session.Initialize(System.Type)
```
Initialize the application session and get the program profile data.(初始化应用程序会话，并获取应用程序的配置数据)

#### TryGetShoalShellBin
```csharp
Cytoscape.Settings.Session.TryGetShoalShellBin
```
