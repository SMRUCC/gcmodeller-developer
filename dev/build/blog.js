...
(function (blog) {
     ...
     * 数据库文件总是位于这个位置的
    */
    var dbfile = "/articles/db.json";
    var blogDb;
    function initializeTree() {
        $ts.get(dbfile, function (json) { return buildIndex(json); });
    }
    blog.initializeTree = initializeTree;
    function buildIndex(db) {
        // 默认是按照时间排序的
        var articles = $ts(db.articles).OrderBy(function (a) { return a.time; }).GroupBy(function (a) { return new Date(a.time).getFullYear(); });
        var displays = articles.Select(function (g) {
            var grouptitle = $ts("<h3>").display(g.Key.toString());
            var list = g.Select(function (a) { return $ts("<div>", { class: "" }).display($ts("<a>", { href: "#" + a.url }).display(a.title)); });
            var section = $ts("<div>");
            section.appendChild(grouptitle);
            list.ForEach(function (a) { return section.appendChild(a); });
            return section;
        });
        var main = $ts("#article");
        main.clear();
        displays.ForEach(function (g) { return main.appendChild(g); });
    }
})(blog || (blog = {}));
